...
def test_not_classic():
    """Test generating a NetCDF file from gridded data numpy array without using default CLASSIC format."""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        entry = hf.data_catalog.get_catalog_entry(filter_options)
        hf_hydrodata.netcdf_util.generate_netcdf_file(
            data, entry, filter_options, file_name, netcdf_format="NETCDF4"
        )
        ds = xr.open_dataset(file_name)
        assert os.path.exists(file_name)
        assert len(ds["x"].values) == 52
