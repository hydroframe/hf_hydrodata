"""
Unit test for the data_access module.
"""
...
import hf_hydrodata.data_access
...
def test_get_vegp():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    hf_hydrodata.data_access.get_raw_file(
        filepath="./vegp.dat",
    ...
    with patch(
        ...
        hf_hydrodata.data_access.HYDRODATA = "/empty"
        hf_hydrodata.data_access.get_raw_file(
            filepath="./vegp.dat",
            dataset="conus1_baseline_mod",
            file_type="vegp",
...
def test_start_time_in_get_ndarray():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    entry = hf_hydrodata.data_access.get_catalog_entry(
...
def test_get_numpy_data():
    """Test the get_numpy_data helper method"""

    data = hf_hydrodata.data_access.get_numpy_data(
        dataset="NLDAS2",
        file_type="pfb",
        period="hourly",
...
def test_get_date_dimension_pfb():
    ...
    end_time = start_time + datetime.timedelta(hours=96)
    dates = []
    hf_hydrodata.data_access.get_ndarray(
...
def test_get_entries():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    rows = hf_hydrodata.data_access.get_catalog_entries(
...
def test_get_table_rows():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    rows = hf_hydrodata.data_access.get_table_rows(
        "variable", variable_type="atmospheric"
    )
    assert len(rows) >= 8

    rows = hf_hydrodata.data_access.get_table_rows("variable", variable_type="land_use")
    assert len(rows) == 0
...
def test_get_table_row():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    entry = hf_hydrodata.data_access.get_table_row("grid", id="conus1")
    assert entry is not None

    entry = hf_hydrodata.data_access.get_table_row("grid", id="conus5")
    assert entry is None

    with pytest.raises(ValueError):
        entry = hf_hydrodata.data_access.get_table_row(
...
def test_get_paths_and_metadata():
    ...
    row = hf_hydrodata.data_access.get_catalog_entry(options)
    paths = hf_hydrodata.data_access.get_file_paths(row, options)
...
def test_get_yaml_data_catalog():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    data_catalog = data_catalog = hf_hydrodata.data_access.get_yaml_data_catalog()
...
def test_files_exist():
    ...
    for row in rows:
        ...
        if not row_id in ["206", "207", "208", "209", "210", "213", "253", "254"]:
            paths = hf_hydrodata.data_access.get_file_paths(
                row,
...
def test_get__tables():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    table_names = hf_hydrodata.data_access.get_table_names()
...
def test_vegm():
    ...
    data = hf_hydrodata.data_access.get_ndarray(entry, grid_bounds=grid_bounds)
...
def test_get_ndarray_conus1_domain_porosity():
    ...
    data = hf_hydrodata.data_access.get_ndarray(entry, grid_bounds=bounds)
...
def test_get_ndarray_tiff():
    ...
    data = hf_hydrodata.data_access.get_ndarray(entry, grid_bounds=bounds, level=4)

    assert data.shape == (50, 100)

    data = hf_hydrodata.data_access.get_ndarray(entry, level=4)
...
def test_get_ndarray_latlng():
    ...
    data = hf_hydrodata.data_access.get_ndarray(entry)
    assert data.shape == (1888, 3342)
...
def test_grid_to_latlng():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    (lat, lng) = hf_hydrodata.data_access.grid_to_latlng("conus1", 0, 0)
    assert round(lat, 2) == 31.65
    assert round(lng, 2) == -115.98
    bounds = hf_hydrodata.data_access.grid_to_latlng("conus1", *[0, 0, 3341, 1887])
    assert round(bounds[2], 2) == 49.1
    assert round(bounds[3], 2) == -76.11
    (lat, lng) = hf_hydrodata.data_access.grid_to_latlng("conus1", 10.5, 10.5)
    assert round(lat, 6) == 31.764588
    assert round(lng, 6) == -115.898577
    (lat, lng) = hf_hydrodata.data_access.grid_to_latlng("conus1", 10.0, 10.0)
    assert round(lat, 6) == 31.759219
    assert round(lng, 6) == -115.902573
    (lat, lng) = hf_hydrodata.data_access.grid_to_latlng("conus2", 0, 0)
...
def test_latlng_to_grid():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    (x, y) = hf_hydrodata.data_access.latlng_to_grid("conus1", 31.759219, -115.902573)
    assert x == 10
    assert y == 10
    grid_bounds = hf_hydrodata.data_access.latlng_to_grid(
    ...
    (x, y) = hf_hydrodata.data_access.latlng_to_grid("conus1", 49.1423, -76.3369)
    assert x == 3324
    assert y == 1888
...
def test_get_huc_from_point():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    grid = "conus1"
    (lat, lng) = hf_hydrodata.data_access.grid_to_latlng("conus1", 1078, 722)
    huc_id = hf_hydrodata.data_access.get_huc_from_latlng(grid, 10, lat, lng)
    assert huc_id == "1019000404"

    huc_id = hf_hydrodata.data_access.get_huc_from_xy(grid, 10, 1078, 722)
    assert huc_id == "1019000404"

    huc_id = hf_hydrodata.data_access.get_huc_from_xy(grid, 10, 1078, 1999)
...
def test_get_huc_bbox_conus1():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    with pytest.raises(ValueError):
        hf_hydrodata.data_access.get_huc_bbox("bad grid", ["1019000404"])
    with pytest.raises(ValueError):
        hf_hydrodata.data_access.get_huc_bbox("conus1", ["1019000404", "123"])

    bbox = hf_hydrodata.data_access.get_huc_bbox("conus1", ["1019000404"])
    assert bbox == (1076, 720, 1124, 739)

    bbox = hf_hydrodata.data_access.get_huc_bbox("conus1", ["1102001002", "1102001003"])
...
def test_get_huc_bbox_conus2():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    bbox = hf_hydrodata.data_access.get_huc_bbox("conus2", ["1019000404"])
...
def test_latlng_to_grid_out_of_bounds():
    ...
    hf_hydrodata.data_access.HYDRODATA = "/hydrodata"
    (_, y) = hf_hydrodata.data_access.latlng_to_grid("conus1", 90, -180)
    assert y > 1888
...
def test_get_ndarray_no_entry_passed():
    ...
    path = hf_hydrodata.data_access.get_file_path(
        None,
        dataset="NLDAS2",
        file_type="pfb",
        period="daily",
...
def test_get_numpy_monthly():
    ...
    data = hf_hydrodata.data_access.get_numpy_data(options)
    assert data.shape == (2, 1888, 3342)
...
def test_get_entry_with_multiple_file_types():
    ...
    entries = hf_hydrodata.data_access.get_catalog_entries(options)
    assert len(entries) > 1
    entry = hf_hydrodata.data_access.get_catalog_entry(options)
